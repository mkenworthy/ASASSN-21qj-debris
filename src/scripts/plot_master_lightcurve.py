...
def bin_neowise(data):
    ...
    """
    bincenters = np.unique(50*np.round(data.time.mjd/50, decimals=0))
    start_time = Time(bincenters-50, format="mjd")
    end_time = Time(bincenters+50, format="mjd")
...
def masterplot_paper(neowise, aavso_b, aavso_v, aavso_i, asassn_v, asassn_g, atlas_c, atlas_o, lcogt_g, lcogt_i, lcogt_r):
    ...
    #pad = 1.4 * (leg.borderaxespad + leg.borderpad) * fontsize
    #leg._legend_box.set_height(leg.get_bbox_to_anchor().height-pad)
    #pf.save_figure(fig, subdir="/lightcurves/master_lightcurve/", filename="master_curve_paper", ext=".pdf", dpi=300)
    fig.savefig(paths.figures / 'master_lightcurve_rvc.pdf')
